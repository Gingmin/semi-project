-- 기존 테이블 삭제
DROP TABLE TBL_ATTACH_VIDEO;
DROP TABLE TBL_ATTACH_LICENSE;
DROP TABLE TBL_ATTACH_NOTICE;
DROP TABLE TBL_ATTACH_CLASS;
DROP TABLE TBL_ATTACH_REVIEW;
...
COMMENT ON COLUMN TBL_ATTACHMENT.ORIGINAL_NAME IS '첨부파일명';
COMMENT ON COLUMN TBL_ATTACHMENT.FILE_NAME IS '변환된이름';
COMMENT ON COLUMN TBL_ATTACHMENT.UPLOAD_DATE IS '업로드일시';
COMMENT ON COLUMN TBL_ATTACHMENT.THUMBNAIL_PATH IS '썸네일경로';
...
CREATE TABLE TBL_REALTIME_GROUP_CLASS(
  ...
  OPENED_CLASS_NO NUMBER REFERENCES TBL_OPENED_CLASS(OPENED_CLASS_NO),
...
CREATE TABLE TBL_CLASS_REVIEW(
  ...
  CONSTRAINT PK_CR PRIMARY KEY (OPENED_CLASS_NO, MEMBER_NO)
);
...
-- 첨부파일관계 테이블 생성
-- 녹화영상관계
CREATE TABLE TBL_ATTACH_VIDEO(
  VIDEO_NO NUMBER REFERENCES TBL_RECORD_VIDEO_MANAGE(VIDEO_NO),
  ATTACHMENT_NO NUMBER REFERENCES TBL_ATTACHMENT(ATTACHMENT_NO),
  CONSTRAINT PK_VA PRIMARY KEY (VIDEO_NO, ATTACHMENT_NO)
);
COMMENT ON TABLE TBL_ATTACH_VIDEO IS '첨부녹화영상관계';
COMMENT ON COLUMN TBL_ATTACH_VIDEO.VIDEO_NO IS '영상번호';
COMMENT ON COLUMN TBL_ATTACH_VIDEO.ATTACHMENT_NO IS '첨부파일번호';
-- 트레이너자격증관계
CREATE TABLE TBL_ATTACH_LICENSE(
  LICENSE_CODE VARCHAR2(50) REFERENCES TBL_LICENSE(LICENSE_CODE),
  ATTACHMENT_NO NUMBER REFERENCES TBL_ATTACHMENT(ATTACHMENT_NO),
  CONSTRAINT PK_LA PRIMARY KEY (LICENSE_CODE, ATTACHMENT_NO)
);
COMMENT ON TABLE TBL_ATTACH_LICENSE IS '첨부트레이너관계';
COMMENT ON COLUMN TBL_ATTACH_LICENSE.LICENSE_CODE IS '자격증코드';
COMMENT ON COLUMN TBL_ATTACH_LICENSE.ATTACHMENT_NO IS '첨부파일번호';
-- 게시판관계
CREATE TABLE TBL_ATTACH_NOTICE(
  NOTICE_NO NUMBER REFERENCES TBL_NOTICE(NOTICE_NO),
  ATTACHMENT_NO NUMBER REFERENCES TBL_ATTACHMENT(ATTACHMENT_NO),
  CONSTRAINT PK_NA PRIMARY KEY (NOTICE_NO, ATTACHMENT_NO)
);
COMMENT ON TABLE TBL_ATTACH_NOTICE IS '첨부게시판관계';
COMMENT ON COLUMN TBL_ATTACH_NOTICE.NOTICE_NO IS '자격증코드';
COMMENT ON COLUMN TBL_ATTACH_NOTICE.ATTACHMENT_NO IS '첨부파일번호';
-- 수업관계
CREATE TABLE TBL_ATTACH_CLASS(
  OPENED_CLASS_NO NUMBER REFERENCES TBL_OPENED_CLASS(OPENED_CLASS_NO),
  ATTACHMENT_NO NUMBER REFERENCES TBL_ATTACHMENT(ATTACHMENT_NO),
  CONSTRAINT PK_OA PRIMARY KEY (OPENED_CLASS_NO, ATTACHMENT_NO)
);
COMMENT ON TABLE TBL_ATTACH_CLASS IS '첨부수업관계';
COMMENT ON COLUMN TBL_ATTACH_CLASS.OPENED_CLASS_NO IS '개설수업번호';
COMMENT ON COLUMN TBL_ATTACH_CLASS.ATTACHMENT_NO IS '첨부파일번호';
-- 리뷰관계
CREATE TABLE TBL_ATTACH_REVIEW(
  OPENED_CLASS_NO NUMBER REFERENCES TBL_OPENED_CLASS(OPENED_CLASS_NO),
  MEMBER_NO NUMBER REFERENCES TBL_MEMBER(MEMBER_NO),
  ATTACHMENT_NO NUMBER REFERENCES TBL_ATTACHMENT(ATTACHMENT_NO),
  CONSTRAINT PK_CNA PRIMARY KEY (OPENED_CLASS_NO, MEMBER_NO, ATTACHMENT_NO)
);
COMMENT ON TABLE TBL_ATTACH_REVIEW IS '첨부수업관계';
COMMENT ON COLUMN TBL_ATTACH_REVIEW.OPENED_CLASS_NO IS '개설수업번호';
COMMENT ON COLUMN TBL_ATTACH_REVIEW.MEMBER_NO IS '회원번호';
COMMENT ON COLUMN TBL_ATTACH_REVIEW.ATTACHMENT_NO IS '첨부파일번호';
